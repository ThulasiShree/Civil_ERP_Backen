...
export let CreateJWTToken = (data: any = {}) => {
    ...
    if (data && data['mobileNumber']) {
        tokenData['mobileNumber'] = data['mobileNumber']
    }
  
    const token = jwt.sign(tokenData, 'AstroLove', { expiresIn: '365D' });
    return token;
}
...
export let checkSession = async (req, res, next) => {
    ...
    if (token) {
        ...
        if (headerType.trim() === "Bearer") {
            try {
                jwt.verify(tokenValue, 'AstroLove', function (err, tokendata) {
                    if (err) {
                    ...
                    if (tokendata) {
                        ...
                        console.log("session error we check");
                    }
                });
            } catch (err: any) {
